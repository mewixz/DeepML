...
def evalSemiParametricCorrections(prediction):

    """wrapper to get a random value and the peak out of the regressed semi-parametric PDF"""
    
    paramList = {}
    paramList['mu']     = rangeTransform(prediction[0],-3,3)
    paramList['sigma']  = rangeTransform(prediction[1],1e-3,5)
    paramList['sigmaL'] = rangeTransform(prediction[2],1e-3,10)
    paramList['fL']     = rangeTransform(prediction[3],1e-6,1)
    paramList['sigmaR'] = rangeTransform(prediction[4],1e-3,10)
    paramList['fR']     = rangeTransform(prediction[5],1e-6,1)
    prf_lne1            = ParametricRecoilFunction('bgsum',paramList)
    paramList['mu']     = rangeTransform(prediction[6],-0.5*ROOT.TMath.Pi(),0.5*ROOT.TMath.Pi())
    paramList['sigma']  = rangeTransform(prediction[7],1e-3,ROOT.TMath.Pi())
    paramList['sigmaL'] = rangeTransform(prediction[8],1e-3,1e4)
    paramList['fL']     = rangeTransform(prediction[9],1e-6,1.0)
    paramList['sigmaR'] = rangeTransform(prediction[10],1e-3,1e4)
    paramList['fR']     = rangeTransform(prediction[11],1e-6,1.0)
    prf_phi             = ParametricRecoilFunction('bgsum',paramList,valRange=(-ROOT.TMath.Pi(),ROOT.TMath.Pi()))
   

    return [prf_lne1.getRandom(-6,6), 
